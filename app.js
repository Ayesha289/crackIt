...
async function main() {
  await mongoose.connect(
    "mongodb+srv://" +
      ...
      "@cluster0.5nfxrlm.mongodb.net/crackitDB"
  );
}
...
app.post("/login", function (req, res) {
  ...
  req.login(user, function (err) {
    if (err) {
      ...
      passport.authenticate("local")(req, res, function () {
        req.session.usermails = req.body.username;
        res.redirect("/quiz");
      });
    }
  });
  ...
  if (flagone == 0) {
    async function main() {
      ...
      const info = await transporter.sendMail({
        ...
        text:
          "The access code for CodeFiesta - CrackIt Round One is: " +
          roundOneCode,
      });
...
app.post("/quizTwo", async function (req, res) {
  var usermail = req.session.usermails
  if (flagtwo == 0) {
    async function main() {
      const transporter = nodeMailer.createTransport({
        service: "gmail",
      ...
      const info = await transporter.sendMail({
        ...
        subject: "CodeFiesta - CrackIt",
        text:
          "The access code for CodeFiesta - CrackIt Round Two is: " +
          roundTwoCode,
      });
      console.log("Message sent: " + info.messageId);
    }
